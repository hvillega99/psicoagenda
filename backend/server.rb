...
get '/turnosTotales/cantidad/:idPsicologo' do
    ...
    cantidadD = client.query("SELECT COUNT(*) as t_disponibles FROM turnos where estado='disponible' and idPsicologo='#{params[:idPsicologo]}'", :symbolize_keys => true).each
    cantidadO = client.query("SELECT COUNT(*) as t_ocupados FROM turnos where estado='ocupado' and idPsicologo='#{params[:idPsicologo]}'", :symbolize_keys => true).each
    @result = [cantidadT[0],cantidadD[0],cantidadO[0]];
    erb :turnos
end
